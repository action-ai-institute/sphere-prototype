...
class EnipProtocol(Protocol):
    ...
    def _start_server(cls, address, tags):
        ...
        try:
            print "Debug ENIP starting cpppo enip server, address: ",str(address)," tags:", str(tags)
    ...
    def _send_multiple(self, what, value, address='localhost:44818',**kwargs):
        tag_string = []
        for w, v in zip(what,value):
            tag_string.append(EnipProtocol._tuple_to_cpppo_tag(w,v))
        '''
        cmd = shlex.split(
            self._client_cmd +
        ...
        try:
            ...
            with client.connector(host = address, port = '44818', timeout = 100.0) as connection:
                operations = client.parse_operations(tag_string)
                #print 'DEBUG ops: ',str(operations)
                failures,transactions = connection.process(operations=operations, depth = 1, multiple = 53, fragment = False,printing = True, timeout=1.0) 
    ...
    def _receive(self, what, address='localhost:44818', **kwargs):
        ...
        tag_string = []
        tag_string.append(EnipProtocol._tuple_to_cpppo_tag(what))
        '''
        cmd = shlex.split(
            self._client_cmd +
            '--log ' + self._client_log +
        ...
        try:
            ...
            #print 'DEBUG enip receive address, tags: ',address, str(tag_string)
            with client.connector(host = address, port = '44818', timeout = 1.0) as connection:
                operations = client.parse_operations(tag_string)
                #print 'DEBUG ops: ',str(tag_string)
                failures,transactions = connection.process(operations=operations, depth = 1, multiple = 0, fragment = False,printing = True, timeout=1.0) 
...
class ModbusProtocol(Protocol):
    ...
    # This dict is a quick workaround to allow for easy transition between ENIP tags and modbus tags:
    _enip_tag_info = None
    ...
    def _start_server_cmd(cls, cmd_path, address='localhost:502',
        ...
        if sys.platform.startswith('linux'):
            ...
            raise OSError
        # Convert tags if they are in ENIP format:
    ...
    def _tuple_to_pymodbus_tags(cls, tags, serializer=':'):
        ...
        """
        # Quick workaround to convert ENIP tags to Modubs tags
        if type(tags[0]) not int:
            _enip_tag_info = OrderedDict()
            DI = 0
            CO = 0
            IR = 0
            HR = 0
            for tag in tags:
                if tag[2] == 'INT' or tag[2] == 'REAL':
                    _enip_tag_info[str(tag[0])+str(tag[1])] = ('HR',HR,tag[2])
                    HR = HR + 2
    ...
    def _send_multiple(self, what, value, address='localhost:502',**kwargs):
        for w, v in zip(what, value):
            self._send(w, v, address)
    ...
    def _send(self, what, value, address='localhost:502', **kwargs):
        ...
        """
        # Workaround for quickly converting enip tag to modbus tag
        if _enip_tag_info is not None:
            enip_tag = _enip_tag_info[str(what[0][0] + what[0][1])]
            what[0] = enip_tag[0]
            what[1] = enip_tag[1]
            # build payload
        ...
        try:
            ...
            if what[0] == 'HR' or what[0] == 'IR':
                ...
                # NOTE: multiple reads returns a list of ints
                    ...
                    if enip_type is not None:
                        decoder = BinaryPayloadDecoder.fromRegisters(raw_string[1:-1].split(','),Endian.Big, wordorder=Endian.Little)
                        if enip_type == "INT":
                            out = decoder.decode_32bit_int()
                        else:
